...
T6W7 L1 PPTX is confirmed clean in real PowerPoint (v12) after five separate root-caused defects fixed across an extended repair-dialog debugging loop (see "What's been produced" for the writeup location — do not re-investigate these, they're closed). T6W7 L1's Learning Paper (LP1, sorting task) is built, QA-rendered, and delivered, including a v2 revision that added the particle-model reference diagram to Part A after Innes's content-principle feedback (see "Decisions locked in") — not yet confirmed by Innes in real PowerPoint.
...
- GitHub `Science/build_t6w7_l1_lp.py` — LP builder for T6W7 L1, python-pptx based, uses the real `label_builder.py` pipeline (not hand-rolled). Pattern copied from `Geography/build_geo_lps_pptx_v3.py`. Contains two reusable helpers — **use both for any future LP builder**: `_wrap_line_count()` (see "Decisions locked in") and `add_reference_image()` (places an image at native aspect ratio, capped height, centred — for the resource-on-the-page rule below).
- GitHub `Science/T6W7_L1_LP1_Scientist_States_of_Matter.pptx` — delivered LP (2 slides: pupil page + Marking Station), v2 with particle-model diagram added to Part A.
- GitHub `LearningPaper/LP_CONTENT_PRINCIPLES.md` — new, cross-subject content rules for LPs (distinct from `label-spec.md`, which is measurements only). Currently one rule: if task text names a resource (particle model, map, timeline, etc.), that resource must actually be on the page, not just referenced. Read this before writing content for any future LP, any subject.
...
- **Content rule, standing**: if an LP's task text names a specific resource (particle model, map, diagram, timeline, photos), that resource must be placed on the LP itself, not just referenced in words. Full writeup in `LearningPaper/LP_CONTENT_PRINCIPLES.md` — apply this to every future LP, any subject, not just science.
