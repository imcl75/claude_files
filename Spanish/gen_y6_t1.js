...
const DATA = require('./y6_t1_data.js');

// ── Colours ───────────────────────────────────────────────────────────────────
const GREEN      = '2bae62';   // Y6 Hazel brand (green)
const AMBER      = 'FFC000';   // WFA accent
const BG         = 'DEECF8';   // WFA pale blue
const WHITE      = 'FFFFFF';
const DARK       = '2D3142';
const TILE_BG    = 'D6EAF8';   // alphabet tile fill (standard)
const TILE_FG    = '1A5276';   // alphabet tile text
const TILE_BD    = 'A9CCE3';   // alphabet tile border
const GREEN_PALE = 'C8EDD8';   // soft green for secondary text
const GREEN_DARK = '1F8A4A';   // darker green for accents
...
function addHeader(slide, label) {
  slide.addShape('rect', {x:0,y:0,w:13.3,h:0.82, fill:{color:GREEN}, line:{width:0}});
  slide.addText(label, {
...
function addTitle(pres, lesson) {
  ...
  slide.addShape('rect', {x:0,y:0,w:8.0,h:7.5, fill:{color:GREEN}, line:{width:0}});
...
function addLO(pres, lesson) {
  ...
  slide.addShape(rr(pres), {
    ...
    fill:{color:GREEN}, line:{color:AMBER,width:3}, rectRadius:0.1,
  });
  ...
  slide.addText('Can you do this by the end of today?', {
    ...
    fontSize:16, italic:true, fontFace:FB, color:GREEN,
...
function addVowels(pres) {
  ...
  vows.forEach((v,i) => {
    ...
    slide.addShape(rr(pres), {
      ...
      fill:{color:GREEN}, line:{color:AMBER,width:4}, rectRadius:0.12,
    });
    slide.addText(v.l, {
...
function addAlphabet(pres, lesson) {
  ...
  ALPHA.forEach((lt, idx) => {
    ...
    const fillC = isPh ? GREEN  : (isVow ? AMBER  : TILE_BG);
    const textC = isPh ? WHITE  : (isVow ? DARK   : TILE_FG);
    const bordC = isPh ? AMBER  : (isVow ? GREEN  : TILE_BD);
    const bordW = isPh ? 3 : (isVow ? 2 : 1);
    const subC  = isPh ? GREEN_PALE : (isVow ? '5D4E00' : '6090B0');
  ...
  const legY = 6.6;
  slide.addShape('rect', {x:0.3,y:legY,w:0.22,h:0.22, fill:{color:GREEN}, line:{width:0}});
  slide.addText("Today's sound", {x:0.56,y:legY-0.02,w:2.0,h:0.26, fontSize:11, fontFace:FB, color:GREEN});
  ...
  slide.addShape(rr(pres), {x:5.1,y:legY,w:0.22,h:0.22, fill:{color:GREEN}, line:{width:0}, rectRadius:0.11});
...
function addWarmUp(pres, lesson) {
  ...
  slide.addShape(rr(pres), {
    ...
    fill:{color:GREEN}, line:{width:0}, rectRadius:0.1,
  });
  ...
  slide.addShape(rr(pres), {
    ...
    fill:{color:WHITE}, line:{color:GREEN,width:2}, rectRadius:0.1,
  });
  slide.addText(lesson.warmUp.sentence1, {
    x:0.7,y:3.4,w:11.9,h:1.0,
    fontSize:22, bold:true, fontFace:FH, color:GREEN,
    valign:'middle', align:'center', wrap:true,
  });
  slide.addShape(rr(pres), {
...
function addVocab(pres, lesson) {
  ...
  const cH = Math.ceil(vocab.length / COLS) >= 2 ? 2.3 : 3.0;
  const gY = 0.18;
  ...
  vocab.forEach((v, i) => {
    ...
    slide.addShape('rect', {x,y,w:cW,h:0.09, fill:{color:GREEN}, line:{width:0}});
    slide.addText(v.sp, {
    ...
    slide.addText(v.eng, {
      ...
      fontSize:16, fontFace:FB, color:GREEN,
      valign:'middle', align:'center', wrap:true,
    });
  });
  ...
  slide.addShape(rr(pres), {
    ...
    fill:{color:GREEN}, line:{width:0}, rectRadius:0.08,
  });
...
function addCultural(pres, lesson, imgData) {
  ...
  if (imgData) {
    ...
    slide.addShape('rect', {x:8.5,y:0,w:4.8,h:7.5, fill:{color:GREEN}, line:{width:0}});
    ...
    slide.addText(c.detail, {
      ...
      fontSize:17, fontFace:FB, color:GREEN_PALE,
      valign:'top', align:'left', wrap:true,
    });
  } else {
    // Card fallback for review lessons
    ...
    vocab.slice(0,6).forEach((v,i) => {
      ...
      slide.addShape(rr(pres), {
        x,y,w:cW,h:cH,
        fill:{color:even?GREEN:WHITE}, line:{color:AMBER,width:2}, rectRadius:0.1,
      });
      slide.addText(v.sp, {
      ...
      slide.addText(v.eng, {
        ...
        fontSize:17, fontFace:FB, color:even?GREEN_PALE:GREEN,
        valign:'middle', align:'center', wrap:true,
      });
    });
    slide.addText('Cover the English — can you remember all the Spanish words?', {
      x:0.5,y:6.6,w:12.3,h:0.55,
      fontSize:15, italic:true, fontFace:FB, color:GREEN,
      valign:'middle', align:'center',
    });
  }
}
...
function addActivity(pres, lesson, idx) {
  ...
  if (act.icon) {
    slide.addShape('ellipse', {
      ...
      fill:{color:GREEN}, line:{color:AMBER,width:3},
    });
  ...
  if (act.examples && act.examples.length) {
    act.examples.forEach((ex, i) => {
      ...
      slide.addShape(rr(pres), {
        ...
        fill:{color:even?GREEN:WHITE}, line:{color:GREEN,width:2}, rectRadius:0.08,
      });
      slide.addText(ex, {
...
function addPhonics(pres, lesson) {
  ...
  slide.addShape(rr(pres), {
    ...
    fill:{color:GREEN}, line:{color:AMBER,width:5}, rectRadius:0.15,
  });
  ...
  slide.addShape('rect', {x:0.45,y:4.05,w:3.6,h:1.5, fill:{color:AMBER}, line:{width:0}});
  ...
  lesson.phonicsExamples.forEach((ex, i) => {
    ...
    slide.addShape(rr(pres), {
      ...
      fill:{color:even?GREEN:'B3DEC6'}, line:{color:AMBER,width:1}, rectRadius:0.08,
    });
...
function addRoundOff(pres, lesson) {
  ...
  slide.addShape('rect', {x:0,y:0,w:13.3,h:7.5, fill:{color:GREEN}, line:{width:0}});
  ...
  slide.addShape(rr(pres), {
    ...
    fill:{color:GREEN_DARK}, line:{color:AMBER,width:2}, rectRadius:0.1,
  });
  ...
  slide.addText('Wallscourt Farm Academy · Spanish · Y6 · Aut 1', {
...
async function buildLesson(lesson) {
  ...
  let imgData = null;
  const imgPath = `/home/claude/y6_l0${lesson.num}_img.jpg`;
  if (fs.existsSync(imgPath)) {
    const buf = fs.readFileSync(imgPath);
    imgData = 'image/jpeg;base64,' + buf.toString('base64');
    console.log(`  Image loaded: y6_l0${lesson.num}_img.jpg`);
  } else {
    console.log(`  No image — card fallback`);
  }

  addTitle(pres, lesson);
  addLO(pres, lesson);
  addVowels(pres);
  addAlphabet(pres, lesson);
  addCounting(pres, lesson);
  addWarmUp(pres, lesson);
  addVocab(pres, lesson);
  addCultural(pres, lesson, imgData);
  addActivity(pres, lesson, 0);
  addActivity(pres, lesson, 1);
  addPhonics(pres, lesson);
  addActivity(pres, lesson, 2);
  addRoundOff(pres, lesson);
