...
// ─── maths_visuals.py bridge ──────────────────────────────────────────────────
// Renders any maths_visuals spec to a temp PNG the LP slide can addImage().
// Spec keys map directly to the 50 types in maths_visuals.py — no changes
// needed here when new types are added.
//
// LP question spec (in lesson_data.js):
//   { q: "Write the fraction shown.",
//     visual: { type: "fraction_shape", shape_type: "circle",
//               denominator: 5, numerator: 3, color: "#C83030" },
//     visual_height: 0.9,   // inches — default 0.80
//     answer: "3/5" }
//
// Supported visual_position: "above" (default and only option for now).
// Adding new visual types → just use the new type key in lesson_data.js.

const _tmpVisuals = [];

function _renderVisual(spec, dpi) {
  dpi = dpi || 150;
  const tmpPath = path.join(os.tmpdir(),
    "lp_vis_" + Date.now() + "_" + Math.random().toString(36).slice(2) + ".png");
  try {
    execFileSync(
      "python3",
      ["/home/claude/render_visual_cli.py", tmpPath, String(dpi)],
      { input: JSON.stringify(spec), encoding: "utf8", timeout: 15000 }
    );
    _tmpVisuals.push(tmpPath);
    return tmpPath;
  } catch (e) {
    var msg = (e.stderr || e.message || "").split("\n")[0];
    console.warn("  ⚠ _renderVisual failed:", msg);
    return null;
  }
}
...
(async () => {
  ...
  // ── Delete temp visual PNGs generated by _renderVisual() ─────────────────
  _tmpVisuals.forEach(function(p) { try { fs.unlinkSync(p); } catch(_e) {} });
  _tmpVisuals.length = 0;
...
function _buildCompactWithData(slide, isMarkingStation, data) {
  ...
  // estimateQH now takes the full question object so it can account for visuals
  function estimateQH(q) {
    var text = (typeof q === 'string') ? q : q.q;
    ...
    const textH = lines * (FS / 72 * 1.25) + 0.04;
    if (!q || !q.visual) return textH;
    // Visual above text: add image height + small gap
    return (q.visual_height || 0.80) + 0.08 + textH;
  }

  const qHeights    = questions.map(function(q) { return estimateQH(q); });
  ...
  for (let rep = 0; rep < repsPerPage; rep++) {
    ...
    for (let qi = 0; qi < nQ; qi++) {
      ...
      // ── Visual (optional) — rendered above question text ─────────────────
      var yText = y;
      if (q.visual) {
        var visH = q.visual_height || 0.80;
        var tmpPath = _renderVisual(q.visual);
        if (tmpPath) {
          slide.addImage({ path: tmpPath, x: Q_X, y: y, w: Q_W, h: visH });
          yText = y + visH + 0.08;
        }
      }

      // ── Question text ───────────────────────────────────────────────────
      // textH = qH minus any visual space already consumed above
      var textH = qH - (yText - y);
      slide.addText('Q' + (qi + 1) + ')   ' + q.q, {
        x: Q_X, y: yText, w: Q_W, h: Math.max(textH, 0.18), fontSize: FS, fontFace: FONT_C,
        color: isMarkingStation ? BLACK : '1F4E79',
        valign: 'top', margin: 0, wrap: true
      });
...
function _buildLP1WordProblems(slide, isMarkingStation) {
  ...
  // If any question has a visual, reserve space for the tallest one + gap
  const maxVisH  = hasVisuals
    ? Math.max(...questions.map(q => q.visual ? (q.visual_height || 0.80) : 0))
    : 0;
  const VIS_GAP  = hasVisuals ? 0.07 : 0;
  const Q_TEXT_H = Math.min(MAX_Q_H, stripH - ANS_H - ANS_RES - maxVisH - VIS_GAP - 0.02);
  ...
  for (let rep = 0; rep < repsPerQ; rep++) {
    ...
    for (let qi = 0; qi < nQ; qi++) {
      ...
      // ── Visual — rendered above question text ──────────────────────────────
      let qTextY = stripTop;
      if (q.visual) {
        const visH = q.visual_height || 0.80;
        const tmpPath = _renderVisual(q.visual);
        if (tmpPath) {
          slide.addImage({ path: tmpPath, x: MARGIN, y: stripTop, w: contentW, h: visH });
          qTextY = stripTop + visH + VIS_GAP;
        }
      }

      // ── Question text ───────────────────────────────────────────────────────
      slide.addText(`Q${qi + 1})   ${q.q}`, {
        x: MARGIN, y: qTextY, w: contentW, h: Q_TEXT_H,
      ...
      if (isTypeA && !isMarkingStation && ANS_H > 0) {
        ...
        slide.addText(prompt, {
          x: MARGIN, y: qTextY + Q_TEXT_H + 0.01,
          w: contentW, h: ANS_H,
