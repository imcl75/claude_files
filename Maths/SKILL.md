...
#### Fix OOXML issues (prevents PowerPoint repair dialog)

Run this over all teaching PPTXs after LP preview injection, before packaging. Fetches `fix_pptx_ooxml.py` from GitHub if not already present.
...
teaching_files = glob.glob('/home/claude/*_Teaching.pptx')
if teaching_files:
    subprocess.run(['python3', '/home/claude/fix_pptx_ooxml.py'] + teaching_files, check=True)
```

Then package into zip and call `present_files`.
