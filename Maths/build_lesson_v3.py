...
# ===========================================================================
# COLUMN CALCULATION SLIDE — short multiplication and short division
# ===========================================================================
def draw_column_calc_slide(sld, visual_key):
    """
    Draws a formal written column calculation (short × or short ÷) in the
    teaching panel.  Data keys in VISUALS[visual_key]:
      calc_type : 'multiplication' | 'division'
      top       : str  — multiplicand / dividend
      bottom    : str  — multiplier  / divisor
      carries   : str  — carry digits above answer row (spaces as placeholders)
      show_answer : bool
      answer    : str
      caption   : str  — right-column caption
      notes     : str
    """
    v = VISUALS[visual_key]
    calc_type   = v.get('calc_type', 'multiplication')
    top_num     = v.get('top', '')
    bottom_num  = v.get('bottom', '')
    carry_str   = v.get('carries', '')
    show_ans    = v.get('show_answer', True)
    answer      = v.get('answer', '')
  ...
  <p:spPr><a:xfrm><a:off x="{emu(panel_x)}" y="{emu(panel_y)}"/>
    ...
    # ── Column headers: place value labels ────────────────────────────────
    if calc_type == 'multiplication':
        # Determine number of columns needed (answer may be one digit longer)
        top_digits   = top_num.replace(',', '')
        ans_digits   = answer.replace(',', '')
        n_cols       = max(len(top_digits), len(ans_digits)) + 1  # +1 for possible extra
        n_cols       = min(n_cols, 6)
        ...
        for ci in range(n_cols + 1):
            x_div = calc_x + ci * col_w
            add_sp(sld, f'''<p:sp xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main"
                                 xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main">
  <p:nvSpPr><p:cNvPr id="{spid}" name="ColDiv{ci}"/><p:cNvSpPr/><p:nvPr/></p:nvSpPr>
  <p:spPr><a:xfrm><a:off x="{emu(x_div)}" y="{emu(calc_y)}"/>
    <a:ext cx="0" cy="{emu(panel_y + panel_h - calc_y - 0.25)}"/></a:xfrm>
    <a:prstGeom prst="line"><a:avLst/></a:prstGeom>
    <a:ln w="{int(0.75*12700)}"><a:solidFill><a:srgbClr val="DDDDDD"/></a:solidFill></a:ln>
  </p:spPr><p:txBody><a:bodyPr/><a:lstStyle/><a:p/></p:txBody></p:sp>''')
            spid += 1
        ...
        for ci, lbl in enumerate(col_labels):
            add_sp(sld, sp(spid, f'Hdr{ci}',
                           ...
                           col_w, header_h,
                           lbl, font='Aptos', sz=12, bold=True,
                           color='156082', align='ctr', fill='EBF3FB',
                           border=('156082', 0.75), anchor='ctr'))
            spid += 1

        cur_y = calc_y + header_h + 0.08

        # Top number (right-aligned into columns)
        top_digits_clean = top_num.replace(',', '')
        # Pad left so digits align to rightmost columns
        top_padded = top_digits_clean.rjust(n_cols)
        ...
        # Operator (×) in leftmost occupied position of bottom row
        op_y = cur_y + row_h
        add_sp(sld, sp(spid, 'Operator',
                       calc_x, op_y, col_w * 0.8, row_h,
                       '×', font='Aptos', sz=digit_sz, bold=True,
                       color='C00000', align='ctr',
                       fill=None, no_line=True, anchor='ctr'))
        spid += 1

        # Bottom number (right-aligned)
        bot_digits_clean = bottom_num.replace(',', '')
        bot_padded = bot_digits_clean.rjust(n_cols)
        ...
        # Horizontal dividing line (above answer)
        line_y = op_y + row_h + 0.06
        add_sp(sld, f'''<p:sp xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main"
                             xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main">
  ...
  <p:spPr><a:xfrm><a:off x="{emu(calc_x)}" y="{emu(line_y)}"/>
    ...
    <a:ln w="{int(1.5*12700)}"><a:solidFill><a:srgbClr val="333333"/></a:solidFill></a:ln>
        ...
        # Carry digits row (above answer, small)
        if carry_str:
            carry_padded = carry_str.rjust(n_cols)
        ...
        # Answer row
        if show_ans and answer:
            ans_digits_clean = answer.replace(',', '')
            ans_padded = ans_digits_clean.rjust(n_cols)
        ...
        # Bus stop layout
        divisor_str  = str(bottom_num).replace(',', '')
        dividend_str = str(top_num).replace(',', '')
        answer_str   = str(answer).replace(',', '').split('r')[0].strip()
        remainder    = str(answer).split('r')[1].strip() if 'r' in str(answer) else ''
        n_dig        = len(dividend_str)

        col_w    = 0.70
        div_w    = 0.65    # divisor box width
        brk_h    = 0.08    # bracket thickness area
        ...
        carry_sz  = 14
        row_h     = 0.55
        brk_y     = calc_y + row_h        # y of the bracket top bar
        dend_y    = brk_y + brk_h + 0.05  # y of dividend digits
  ...
  <p:spPr><a:xfrm><a:off x="{emu(brk_x)}" y="{emu(brk_y)}"/>
    ...
    <a:ln w="{int(2.0*12700)}"><a:solidFill><a:srgbClr val="333333"/></a:solidFill></a:ln>
        ...
        # Quotient digits above the bracket
        if show_ans and answer_str:
            q_padded = answer_str.rjust(n_dig)
            for di, ch in enumerate(q_padded):
                if ch.strip():
                    add_sp(sld, sp(spid, f'Quot{di}',
                                   brk_x + di * col_w, brk_y - row_h - 0.05,
                                   col_w, row_h,
                                   ch, font='Aptos', sz=digit_sz, bold=True,
                                   color='375623', align='ctr',
                                   fill=None, no_line=True, anchor='ctr'))
        ...
        # Remainder label if present
        if remainder:
            add_sp(sld, sp(spid, 'Remainder',
                           brk_x + n_dig * col_w + 0.10, brk_y - row_h - 0.05,
                           0.80, row_h,
                           f'r {remainder}', font='Aptos', sz=digit_sz - 4, bold=True,
                           color='C00000', align='l',
                           fill=None, no_line=True, anchor='ctr'))
            spid += 1

        # Carry digits row below dividend (small, orange)
        carries = v.get('carries', '')
        if carries:
            carry_padded = carries.rjust(n_dig)
            for di, ch in enumerate(carry_padded):
                if ch.strip():
                    add_sp(sld, sp(spid, f'DivCarry{di}',
                                   brk_x + di * col_w + col_w * 0.5, dend_y + 0.05,
                                   col_w * 0.5, 0.30,
                                   ch, font='Aptos', sz=carry_sz, bold=True,
                                   color='E07000', align='l',
                                   fill=None, no_line=True, anchor='ctr'))
                spid += 1

    # Right column — caption or sentence stem
    right_x = panel_x + panel_w + 0.25
    right_w  = 13.333 - right_x - 0.2
    if caption:
        add_sp(sld, sp(200, 'Caption', right_x, panel_y + 0.15, right_w, 1.40,
                       caption, font='Twinkl Cursive Looped Light', sz=20,
