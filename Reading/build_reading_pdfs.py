...
def draw_header(c, lesson_type, date_str, key_q, lf, ican1, ican2):
    """Draw the learning label header. Returns y after header."""
    y = H - MARGIN

    # Row 1: "Key Question" [icon] date — all on one line left-to-right
    ...
    # Key question — underlined, bold, dark blue
    c.setFont("Helvetica-Bold", 10)
    c.setFillColorRGB(0.173, 0.173, 0.424)
    c.drawString(MARGIN, y - 4*mm, key_q)
    kq_w = c.stringWidth(key_q, "Helvetica-Bold", 10)
    c.setLineWidth(0.5)
    c.setStrokeColorRGB(0.173, 0.173, 0.424)
    ...
    c.setFont("Helvetica", 8)
    c.setFillColorRGB(*DARK)
    c.drawString(MARGIN, y - 3.5*mm, lf)
    y -= 4.5*mm

    # I can lines
    c.drawString(MARGIN, y - 3.5*mm, ican1)
    y -= 4*mm
    c.drawString(MARGIN, y - 3.5*mm, ican2)
    y -= 4.5*mm
...
def draw_order_pupil(c, events, y, correct_order=None):
    """KS2-style sequencing. Pre-fills the item numbered '1' if correct_order supplied."""
    c.setFont("Helvetica", 9)
    c.setFillColorRGB(*DARK)
    box_s = 6*mm
    row_h = 10*mm
    ...
    box_x = MARGIN + 2*mm + max_w + 4*mm
    c.setLineWidth(0.5)
    nums = [n.strip() for n in str(correct_order).split(",")] if correct_order else [None]*len(events)
    for ev, num in zip(events, nums):
        c.setFillColorRGB(*DARK)
        c.setFont("Helvetica", 9)
        ...
        if prefill:
            c.setFillColorRGB(*BOX_BORDER)
            c.setFont("Helvetica-Bold", 9)
            c.drawCentredString(box_x + box_s/2, y - row_h + 4*mm, "1")
            c.setFont("Helvetica", 9)
        y -= row_h + 2*mm
    return y - 1*mm
...
def render_question(c, q, y, is_answer=False, n_lines=3, min_y=20*mm):
    ...
    if qtype == "mc":
        ...
        if is_answer:
            ...
            y = draw_order_pupil(c, options, y, correct_order=correct)
...
merge_pdfs(built["std_pupil"],
           "/mnt/user-data/outputs/T6W5_Standard_Pupil.pdf")

# Supported Pupil: Voc + Ret + Inf
merge_pdfs(built["sup_pupil"],
           "/mnt/user-data/outputs/T6W5_Supported_Pupil.pdf")
...
merge_pdfs(ans_order,
           "/mnt/user-data/outputs/T6W5_All_Answers.pdf")
...
print("Done.")
print("  T5W2_Standard_Pupil.pdf:", PdfReader("/mnt/user-data/outputs/T6W5_Standard_Pupil.pdf").pages.__len__(), "pages")
print("  T5W2_Supported_Pupil.pdf:", PdfReader("/mnt/user-data/outputs/T6W5_Supported_Pupil.pdf").pages.__len__(), "pages")
print("  T5W2_All_Answers.pdf:", PdfReader("/mnt/user-data/outputs/T6W5_All_Answers.pdf").pages.__len__(), "pages")
