...
def build_one_lesson(mtp, lesson_num, base_pptx, out_pptx):
    ...
    # ── Build label sheet ─────────────────────────────────────────────────────
    _label_candidates = [
        os.path.join(_THIS, '..', 'Shared', 'generate_wfa_labels_pdf.py'),
        os.path.join(_THIS, 'generate_wfa_labels_pdf.py'),
    ...
    if _label_script is None:
        ...
        _label_out = os.path.join(
            os.path.dirname(os.path.abspath(out_pptx)),
            os.path.splitext(os.path.basename(out_pptx))[0] + ' Labels.pdf'
        )
        _subject   = lesson_data.get('subject', 'geographer')
        _question  = mtp.get('key_question', '')
        _lf        = lesson_data.get('what', '')
        _success   = lesson_data.get('success', '')
        _parts     = [p.strip() for p in _success.split(',')]
        _ican1     = _parts[0] if len(_parts) > 0 else ''
        _ican2     = ', '.join(_parts[1:]) if len(_parts) > 1 else ''
        _date      = lesson_data.get('lp', {}).get('date', '') if isinstance(lesson_data.get('lp'), dict) else ''
        # Locate ll_assets for icons — prefer sibling of resource_base, else ll_assets next to this script
        _ll_icons_candidates = [
            os.path.join(os.path.dirname(os.path.abspath(out_pptx)), 'll_assets'),
