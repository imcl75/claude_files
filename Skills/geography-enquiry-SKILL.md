...
## LO slide animation — use shape-level reveal, not paragraph-level

The WAL/TIB/ISB placeholders (idx10/13/14) each hold a single paragraph. Animating a single-paragraph shape via paragraph-range targeting (`<p:txEl><p:pRg st="0" end="0"/></p:txEl>`) does not reliably auto-hide on load — PowerPoint may show the text immediately while still consuming the click in the animation sequence, making it look like the slide needs extra clicks to advance with no visible change. Use shape-level targeting instead (`<p:spTgt spid="..."/>` with no `<p:txEl>` wrapper) for any single-paragraph placeholder reveal. Paragraph-range targeting is fine for multi-paragraph bodies (I Do/We Do/You Do slides) where only paragraph 0 stays static and the rest are individually animated — the bug is specific to single-paragraph shapes.

Separately: the "What, Why, How" slide layout (used for every LO slide) has a genuine duplicate set of decorative shapes baked into the layout itself (two overlapping sets of speech-bubble headers and rounded rectangles). This is a template-level issue, not something a slide-level fix can resolve — flag to Innes if a layout cleanup is ever wanted, but don't attempt to edit the shared layout without being asked, since it affects every lesson using it.
