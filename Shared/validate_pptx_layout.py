...
Only validates slides referenced in sldIdLst (not orphaned slide XML files).
...
Usage:
    ...
    python3 validate_pptx_layout.py file.pptx --render    # pixel render check
...
LAYOUT_CONFLICTS = [
    ("learning review", "<p:pic",    "Photo slide on Learning Review layout — layout avatars bleed through"),
    ("learning review", "blipFill",  "Image fill on Learning Review layout — layout avatars bleed through"),
]


def _emu_cm(v): return round(v / 360000, 3)


def _get_canvas(prs_xml):
    if isinstance(prs_xml, bytes): prs_xml = prs_xml.decode()
    root = ET.fromstring(prs_xml)
    sz = root.find(f"{{{PML}}}sldSz")
    if sz is None: return 12192000, 6858000
...
def _get_layout_name(pptx_zip, slide_path):
    ...
    try: rels_xml = pptx_zip.read(rels_path).decode()
    ...
    for rel in root:
        if "slideLayout" in rel.get("Type", ""):
            ...
            try: layout_xml = pptx_zip.read(layout_path).decode()
            except KeyError: return ""
...
def _parse_shapes(slide_xml):
    if isinstance(slide_xml, bytes): slide_xml = slide_xml.decode()
...
def _check_text_spill(shapes):
    ...
    for s in shapes:
        ...
        chars_per_line = max(1, usable_w / 12700 / (s["font_pt"] * CHAR_WIDTH_FACTOR))
...
def _check_collisions(shapes):
    ...
    for i, a in enumerate(candidates):
        for b in candidates[i + 1:]:
            ...
            if smaller and (ox * oy) / smaller >= LAYERING_THRESHOLD: continue
            # Intentional: emoji Icon shapes overlapping Fact card edges in picture_scene
            if (re.search(r"^Icon\s*\d+$", a["name"], re.I) and
                    re.search(r"^Fact\s*\d+$", b["name"], re.I)): continue
            if (re.search(r"^Icon\s*\d+$", b["name"], re.I) and
                    re.search(r"^Fact\s*\d+$", a["name"], re.I)): continue
...
def _check_visual_reference(shapes, slide_xml):
    ...
    if not match: return []
    if _slide_has_image(slide_xml): return []
...
def _render_check(pptx_path):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for i, page in enumerate(pages, 1):
            ...
            if near_white / len(px) > 0.65:
                issues.append({"severity": "WARNING", "type": "RENDER-BLANK-BOTTOM",
                               "slide": f"slide {i}",
                               "detail": f"{near_white/len(px):.0%} of bottom half near-white — possible empty placeholder"})
    return issues


def validate_pptx(pptx_path, include_warnings=True, render=False):
    """
    Validate slides referenced in sldIdLst only.
    Orphaned slide XML files left by build scripts are ignored.
    """
    all_issues = []
    with zipfile.ZipFile(pptx_path, "r") as z:
        ...
        # Read sldIdLst from presentation.xml to find which slides are shown
        referenced = set()
        try:
            prs_xml  = z.read("ppt/presentation.xml").decode()
            prs_rels = z.read("ppt/_rels/presentation.xml.rels").decode()
            rid_to_slide = {}
            for m in re.finditer(r'Id="(rId\d+)"[^>]*Target="slides/(slide\d+\.xml)"', prs_rels):
                rid_to_slide[m.group(1)] = f"ppt/slides/{m.group(2)}"
            for m in re.finditer(r'r:id="(rId\d+)"', prs_xml):
                rid = m.group(1)
        ...
        for sp in slide_files:
            ...
            for issue in issues: issue["slide"] = label
            all_issues += issues

    if render:
        for ri in _render_check(pptx_path):
            if "slide" not in ri: ri["slide"] = "all"
...
def print_report(pptx_path, issues, show_warnings=True):
    ...
    print(f"{'='*65}")
    if not issues:
        print("✅  No layout issues found."); return
    if errors:
        print(f"\n❌  ERRORS ({len(errors)}):")
