...
def make_rect(x, y, w, h, fill_hex, border_hex=None, border_pt=0,
    ...
    wrap_attr = ' wrap="none"' if no_wrap else ''
    ...
    <a:r>
      ...
      <a:t>{safe}</a:t>
        ...
        text_xml = f'''<p:txBody>
  <a:bodyPr rtlCol="0" anchor="{anchor}" lIns="{lins}"{wrap_attr}>{fit_xml}</a:bodyPr>
  <a:lstStyle/>
  {''.join(para_xmls)}
</p:txBody>'''
